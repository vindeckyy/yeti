...
void Hooks::SetViewModelSequence(const CRecvProxyData *pDataConst, void *pStruct, void *pOut) {
	...
	if (pViewModel) {
		IClientEntity* pOwner = m_pEntityList->GetClientEntityFromHandle((HANDLE)pViewModel->GetOwner());

		// Compare the owner entity of this view model to the local player entity.
		if (pOwner && pOwner->GetIndex() == m_pEngine->GetLocalPlayer()) {
			// Get the filename of the current view model.
			model_t* pModel = (model_t*)m_pModelInfo->GetModel(pViewModel->GetModelIndex());
			const char* szModel = m_pModelInfo->GetModelName(pModel);
